...
const App: React.FC = () => {
  ...
  const [animationSpeed, setAnimationSpeed] = useState<AnimationSpeed>(AnimationSpeed.NORMAL);
  ...
  useEffect(() => {
    ...
    gameInstance.onStateChange((updatedGame) => {
      ...
      if (updatedGame.getState() === GameState.FINISHED) {
        ...
        if (result) {
          if (result.isTie) {
            setWinnerMessage("It's a tie! Both players have the same number of columns.");
          } else {
            setWinnerMessage(`Player ${result.winner} wins with ${result.winner === Player.PLAYER1 ? result.player1Columns : result.player2Columns} columns!`);
          }
        }
      } else {
        setWinnerMessage('');
  ...
  const updateGameMessage = useCallback((gameInstance: Game) => {
    ...
    switch (state) {
      ...
      case GameState.PLAYING:
        setGameMessage(`Player ${currentPlayer}'s turn - Click a column to drop a ball`);
        break;
  ...
  const handleColumnClick = useCallback((column: number) => {
    if (game && !isAnimating) {
      try {
        ...
        setGameMessage(error instanceof Error ? error.message : 'Error dropping ball');
      }
    }
  }, [game, isAnimating]);
