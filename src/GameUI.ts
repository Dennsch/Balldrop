...
export class GameUI {
  ...
  private initializeElements(): void {
    ...
    this.selectedMovesDisplay = this.getOrCreateElement(
    ...
    this.animationSpeedSelect = this.getElement(
  ...
  private createGameModeSelector(): void {
    ...
    normalRadio.addEventListener("change", () =>
      this.handleGameModeChange(GameMode.NORMAL)
    );
    ...
    hardRadio.addEventListener("change", () =>
      this.handleGameModeChange(GameMode.HARD_MODE)
    );
    ...
    this.executeMovesButton.addEventListener("click", () =>
      this.handleExecuteMovesClick()
    );
        ...
        const animationPromise = new Promise<void>((resolve) => {
          setTimeout(async () => {
            try {
              ...
              for (
                let stepIndex = 0;
                stepIndex < ballPath.steps.length;
                stepIndex++
              ) {
                ...
                if (
                  ...
                  const boxHitPromise = this.animateBoxHit(
                    step.boxPosition,
                    step.boxDirection
                  );
                ...
                if (
                  step.action === "settle" &&
                  step.position.row === this.game.getGrid().getSize() - 1
                ) {
              ...
              await this.placeFinalBall(
                ballPath.finalPosition,
                ballPath.player
              );
  ...
  private createAnimatedBall(player: Player): HTMLElement {
    ...
    ball.style.transition = `all ${
      0.35 * this.animationTimings[this.currentAnimationSpeed].cssMultiplier
    }s ease-in-out`;
    ball.style.border = "2px solid rgba(255, 255, 255, 0.8)";
    ball.style.boxShadow =
      "0 4px 8px rgba(0, 0, 0, 0.3), 0 0 20px rgba(255, 255, 255, 0.5)";
  ...
  private async animateToPosition(
    ...
    return new Promise((resolve) => {
      ...
      if (action === "redirect") {
        ...
        ball.style.boxShadow =
          "0 4px 8px rgba(0, 0, 0, 0.3), 0 0 30px rgba(255, 255, 0, 0.8)";
        setTimeout(() => {
      ...
      const baseDuration =
        action === "settle" ? 250 : action === "redirect" ? 400 : 350;
  ...
  private async animateBoxDirectionChange(
  ...
  private async placeFinalBall(
    ...
    return new Promise((resolve) => {
      ...
      if (cellElement) {
        ...
        const ballClass =
          player === Player.PLAYER1 ? "has-ball-p1" : "has-ball-p2";
  ...
  private updateColumnSelectors(): void {
    const buttons =
      this.columnSelectorsElement.querySelectorAll(".column-selector");
    const gameMode = this.game.getGameMode();
    const state = this.game.getState();
  ...
  private updateGameStatus(): void {
    ...
    if (state === GameState.FINISHED) {
      ...
      if (result.isTie) {
        ...
        this.winnerMessageElement.textContent = `It's a tie! Both players control ${this.sanitizeInput(
          result.player1Columns
        )} columns`;
      } else if (result.winner === Player.PLAYER1) {
        this.winnerMessageElement.classList.add("player1-wins");
        this.winnerMessageElement.textContent = `Player 1 wins! Controls ${this.sanitizeInput(
          result.player1Columns
        )} columns vs ${this.sanitizeInput(result.player2Columns)}`;
      } else if (result.winner === Player.PLAYER2) {
        this.winnerMessageElement.classList.add("player2-wins");
        this.winnerMessageElement.textContent = `Player 2 wins! Controls ${this.sanitizeInput(
          result.player2Columns
        )} columns vs ${this.sanitizeInput(result.player1Columns)}`;
      }

      this.gameMessageElement.textContent =
        'Game finished! Click "New Game" to play again.';
    } else {
      this.winnerMessageElement.classList.add("hidden");

      if (state === GameState.SETUP) {
        this.gameMessageElement.textContent =
          'Welcome to Balldrop! Click "New Game" to start playing.';
